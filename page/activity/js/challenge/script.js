...
while (sortedNames.length > 0) {
  ...
  if (DEBUG) console.log('Checking ' + currentStudent + ' against ' + sortedNames.length + ' others');
  ...
  sortedNames.forEach(function(name, idx) {
    ...
    if (DEBUG) console.log(' ' + (idx + 1) + '. Checking against ' + name);

    // If the name starts with a vowel, use hamming distance, otherwise use levenshtein
    if (/^[AEIOU].+/.test(name)) {
      if (DEBUG) console.log('    Starts with a vowel (Hamming distance)');
      distance = {
        name,
        distance: hamming.distance(currentStudent, name)
      }
    } else {
      if (DEBUG) console.log('    Starts with a consonant (Levenshtein distance)');
      distance = {
        name,
        distance: levenshtein.distance(currentStudent, name)
      }
    }
    if (DEBUG) console.log('    Distance is ' + distance.distance);
  ...
  if (DEBUG && lowestDistance.name !== '') console.log(lowestDistance.name + ' removed ' + (sortedNames.indexOf(lowestDistance.name) === -1 ? 'successfully' : 'unsuccessfully'));
  ...
  if (lowestDistance.name !== '') {
    ...
    console.log(currentStudent + ' is unpaired');
  }
}
